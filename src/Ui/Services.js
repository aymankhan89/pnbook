...
const Card = ({ percentage, heading, description }) => (
  <div className="flex gap-[18px]  p-[16px] rounded-[15px]">
    ...
    <div>
      <h2 className="text-[16px] md:text-[20px] font-[500] text-[#F9F9F9] text-left mb-2">
        {heading}
      </h2>
      <p className="text-[#F9F9F9] font-[400] text-[14px] md:text-[16px] text-left">
        {description}
      </p>
    </div>
  </div>
...
const Services = () => {
  const cardData = [
    {
      ...
      description:
        "Detailed tracking and recording of all financial transactions, ensuring accuracy and up-to-date financial records.",
    },
    {
    ...
    },
    {
      ...
      description:
        "Assistance with tax filing and compliance, including preparation of tax returns and ensuring adherence to relevant tax regulations.",
    },
    {
  ...
  const mainDescription =
    "Discover our diverse range of professional services, expertly designed to address your unique business needs and drive growth and efficiency.";
  ...
  const getHighlightedText = (text, highlights) => {
    ...
    return parts.map((part, index) =>
      highlights.includes(part) ? (
        <span
          key={index}
          className="font-[500] italic text-[#FFC17A] text-[24px] md:text-[40px]"
        >
          {part}
        </span>
      ) : (
  ...
  return (
    <div className="pt-[120px] px-4 sm:px-6 md:px-8">
      <div className="relative w-full flex justify-center">
        <ServicesImg />
        <div className="relative p-4 sm:p-6 md:p-8 flex flex-col md:flex-row items-center justify-between  bg-opacity-90 rounded-[15px] mx-auto max-w-[1200px]">
          <div className="flex-1 flex flex-col justify-center text-left text-white z-10">
            <p className="text-[24px] sm:text-[28px] md:text-[40px] font-[500] mb-4">
              {getHighlightedText(mainHeading, highlightedWords)}
            </p>
            <p className="text-[14px] sm:text-[16px] md:text-[17px] font-[400] mb-8">
              {mainDescription}
            </p>
          </div>
          <div className="flex-1 grid grid-cols-1 gap-[20px] sm:grid-cols-2 lg:grid-cols-2">
